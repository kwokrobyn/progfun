...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences = (for (
      (ch, occ) <- (w.toLowerCase.toList groupBy (c => c))
    ) yield (ch, occ.length)).toList.sorted 
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (y.toMap foldLeft x.toMap) {
      case (accMap, (ch, occ)) => {
        val newOcc = accMap.apply(ch) - occ 
        if (newOcc == 0) accMap - ch else (accMap updated (ch, newOcc))
      }
    }.toList.sorted
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def anaHelper(occsLeft: Occurrences): List[Sentence] = occsLeft match {
      case Nil => List(Nil) 
      case _ => for {
        comb <- combinations(occsLeft)
        word <- dictionaryByOccurrences(comb)
        rest <- anaHelper(subtract(occsLeft, comb))
      } yield word :: rest 
    }
    anaHelper(occs)
